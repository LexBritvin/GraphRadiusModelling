...
//    private static int iterCount = 1;
    ...
    private static double obstacleRadius = 2;
    private static double deviceRadius = 1;

    private static String pathMask = "tests" + File.separator + "expected-radius-%radius-%n.txt";

    public static void main(String[] args) {
        Thread threads[] = new Thread[Main.threadTasks.length];
        File[] files = new File[Main.areaRadius.length];
        for (int i = 0; i < Main.areaRadius.length; i++) {
            double areaRadius = Main.areaRadius[i];
            // Prepare file for results.
            String base_path = Main.pathMask.replace("%radius", String.format("%02d", (int) areaRadius));
            int n = 1;
        ...
        for (int i = 0; i < Main.threadTasks.length; i++) {
            int area_i = Main.threadTasks[i][0];
            double areaRadius = Main.areaRadius[area_i];
            ...
            if (resultsFile != null && resultsFile.canWrite()) {
                CalculateGraphRadiusRunnable r = new CalculateGraphRadiusRunnable(
                        areaRadius,
                        Main.obstacleRadius,
                        Main.deviceRadius,
                        minVertexCount,
                        maxVertexCount,
                        Main.stepVertexCount,
                        Main.iterCount,
                        resultsFile
                );
